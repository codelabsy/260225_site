    ...
    <div class="w-64 flex-shrink-0 bg-white rounded-lg border border-gray-200 shadow-sm overflow-y-auto">
        ...
        <div class="p-4 border-b border-gray-100">
            ...
            <div class="space-y-1" id="status-filter">
                ...
                <button type="button" class="status-filter-btn w-full flex items-center justify-between px-3 py-2 rounded-md text-sm transition-colors" data-status="대기중">
                    <span class="flex items-center"><span class="status-dot status-dot-muted"></span>대기중</span>
                    <span class="badge badge-secondary text-xs" id="count-대기중">0</span>
                </button>
                ...
                <button type="button" class="status-filter-btn w-full flex items-center justify-between px-3 py-2 rounded-md text-sm transition-colors" data-status="거절">
                    <span class="flex items-center"><span class="w-2 h-2 rounded-full bg-orange-500 mr-2"></span>거절</span>
                    <span class="badge badge-secondary text-xs" id="count-거절">0</span>
                </button>
            </div>
        </div>
        ...
        <div class="p-4 border-b border-gray-100">
            ...
            <input type="text" id="filter-search" class="form-input text-sm w-full" placeholder="상호명, 연락처, 상품명">
        </div>
    ...
    <div class="flex-1 flex flex-col min-w-0">
        ...
        <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2">
                <h2 class="text-lg font-semibold text-gray-900">쿠팡 DB</h2>
            ...
            <div class="flex items-center gap-2">
                ...
                <?php endif; ?>
                <button type="button" id="btn-bulk-delete" class="btn btn-danger text-sm" onclick="window.coupangBulkDelete()">
        ...
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm flex-1 overflow-hidden flex flex-col">
            <div class="overflow-x-auto flex-1">
                <table class="w-full text-left" id="coupang-table">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-200">
                            <th class="w-10 px-3 py-3">
                                <input type="checkbox" class="table-check-all rounded border-gray-300 text-blue-600 focus:ring-blue-500" data-table="coupang-table">
                            </th>
                    ...
                    <tbody id="coupang-tbody" class="divide-y divide-gray-100">
                        <tr>
    ...
    <div id="detail-panel" class="w-96 flex-shrink-0 bg-white rounded-lg border border-gray-200 shadow-sm overflow-y-auto hidden">
        <div class="sticky top-0 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between z-10">
            ...
            <div class="flex items-center gap-1">
                <button type="button" id="btn-detail-delete" class="text-red-400 hover:text-red-600 transition-colors rounded-lg p-1 hover:bg-red-50 hidden" onclick="window.coupangDeleteRecord()" title="삭제">
    ...
    <div class="flex items-center justify-center min-h-screen px-4 py-6">
        ...
        <div class="modal-content relative bg-white rounded-lg shadow-xl max-w-lg w-full transform transition-all">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-semibold text-gray-900">엑셀(CSV/XLSX) 업로드</h3>
            ...
            <div class="px-6 py-4">
                ...
                <div id="upload-dropzone" class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-blue-400 transition-colors cursor-pointer"
                    ...
                    <p class="text-sm text-gray-600 mb-1">CSV/XLSX 파일을 여기에 드래그하거나 클릭하여 선택</p>
                    <p class="text-xs text-gray-400">CSV 또는 XLSX 형식 지원</p>
                    <input type="file" id="upload-file-input" accept=".csv,.xlsx" class="hidden">
                </div>
...
<script>
    window.COUPANG_CONFIG = {
        isAdmin: <?= $isAdmin ? 'true' : 'false' ?>,
        currentUserId: <?= $currentUser['id'] ?>,
        statuses: <?= json_encode(COUPANG_STATUSES, JSON_UNESCAPED_UNICODE) ?>,
    };
</script>
<script src="<?= BASE_URL ?>/assets/js/coupang.js?v=<?= time() ?>"></script>
