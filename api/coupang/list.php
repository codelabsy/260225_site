...
if ($status !== '' && in_array($status, COUPANG_STATUSES)) {
...
if ($search !== '') {
    $searchParam = '%' . $search . '%';
    $countWhere[] = '(s.company_name LIKE ? OR s.representative LIKE ? OR s.phone LIKE ? OR s.product_name LIKE ?)';
...
$totalRow = $db->fetch("SELECT COUNT(*) as cnt FROM coupang_db s $totalWhereClause", $countParams);
$total = (int)($totalRow['cnt'] ?? 0);

$filters['limit'] = $pageSize;
$filters['offset'] = ($page - 1) * $pageSize;
$items = CoupangDB::all($filters);
...
if ($search !== '') {
    $searchParam = '%' . $search . '%';
    $statusCountWhere[] = '(s.company_name LIKE ? OR s.representative LIKE ? OR s.phone LIKE ? OR s.product_name LIKE ?)';
...
$statusCountsRaw = $db->fetchAll(
    "SELECT status, COUNT(*) as cnt FROM coupang_db s $countWhereClause GROUP BY status",
    $statusCountParams
);

$statusCounts = [];
foreach (COUPANG_STATUSES as $s) {
    $statusCounts[$s] = 0;
}
$totalAll = 0;
